...
const Final = () => {
  ...
  return (
    <section id="final" className="relative h-screen text-white p-[10%] flex sm:justify-center">
      ...
      <AnimatePresence mode="wait">
        {!btnState && (
          <motion.div key="cta" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 0 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="relative flex flex-col gap-10 self-center">
        ...
        {btnState && (
          <motion.div key="form" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 0 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="relative flex flex-col gap-10 self-center sm:w-3/4 md:w-1/2 lg:w-2/5 xl:w-1/3">
            ...
            <div className="flex flex-col gap-4">
              <input value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="py-5 p-10 rounded-4xl text-sm w-full focus:outline-white outline-2 placeholder:text-white" placeholder="Email Address"/>

              <input value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="py-5 p-10 rounded-4xl text-sm w-full focus:outline-white outline-2 placeholder:text-white" placeholder="Full Name"/>

              <input value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} className="py-5 p-10 rounded-4xl text-sm w-full focus:outline-white outline-2 placeholder:text-white" placeholder="Message"/>

              <button onClick={handleSubmit} className="focus-visible:outline-none focus-visible:ring-2 focus:ring-black font-medium py-5 rounded-4xl whitespace-nowrap lg:text-base text-sm w-full bg-offwhite text-black hover:scale-95 transition ease-out hover:border-white cursor-pointer duration-300">Submit Enquiry</button>
